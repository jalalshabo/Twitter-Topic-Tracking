...
def connect_database(username="root", password="add password here", host="localhost", database="add twitter database", charset="utf8mb4"):
...
class StreamListener(tweepy.StreamListener):
    ...
    def on_data(self, raw_data):
        ...
        try:
            if stream_filter_type_location == True:
                raw_data = json.loads(raw_data)

                # turn created_at which is date into datetime object, then return to string with specified format to
                # ensure that will be accepted by mySQL int datetime format
                ...
                connection.commit()
            elif stream_filtertype_follow == True:
                raw_data = json.loads(raw_data)

                # turn created_at which is date into datetime object, then return to string with specified format to
                # ensure that will be accepted by mySQL int datetime format
                raw_data["created_at"] = "2021-06-09"

                    # datetime.strftime(
                    # datetime.strptime(raw_data["created_at"], '%a %b %d %H:%M:%S +0000 %Y'), '%Y-%m-%d %H:%M:%S')
